...
class ExactInference(object):
    ...
    def observe(self, agentX, agentY, observedDist):
        ...
        for i in range(self.belief.getNumRows()):
            for j in range(self.belief.getNumCols()):
                y,x = util.rowToY(row=i),util.colToX(col=j)
                self.belief.setProb(i, j,
                                    0 if (agentY ==y  and agentX == x) else
                                    self.belief.getProb(i, j) * util.pdf(((agentY - y) ** 2 + (agentX - x) ** 2) ** 0.5,
    ...
    def elapseTime(self):
        ...
        temp_grid = util.Belief(self.belief.getNumRows(), self.belief.getNumCols(),0)
        for old_position,new_position in self.transProb:
            temp_grid.addProb(new_position[0],new_position[1],self.transProb[(old_position,new_position)] * self.belief.getProb(
                old_position[0],old_position[1]
            ))
        self.belief = temp_grid
        self.belief.normalize()
