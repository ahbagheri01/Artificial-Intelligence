...
size_of_population = 2* int(f.readline())
exponential = 2.5
states = []
...
def fitness():
    ...
    for dna_string in states:
        ...
        if temp > maximum:
            ...
            temp = counter if temp <= 0 else temp
            counter+=1
        fit_amount = ((temp+1 if temp >=0 else (-temp)**-1)*(exponential**temp))
        fitness_amount.append(fit_amount)
...
for i in range(n_iteration):
    fitness_cum_amount,maximum,best_string = fitness()
    best_fitness_arr.append(maximum)
    best_string_fitness = best_string
    #selection
    states = random.choices(states,weights= fitness_cum_amount , k=size_of_population)
    #cross over
...
f = open("B"+str(input_number)+".txt", "w")
...
plt.savefig("B"+str(input_number)+".png")


#address E:\term4\AI\practical_exc\1\Q4
